# 📊 SDK Microservice Isolated Logs (`apps/src/sdk/logs/`)
...
| :--- | :--- | :--- |
| **Pillar 1: Dual-Probe Health** | `GET /health` (`livez`, `readyz`) | Confirms process event loop responsiveness and service registry health. |
| **Pillar 2: Browser Console Logs** | `browser.log` | Client-side error bridge forwarding unhandled browser exceptions & network failures. |
| **Pillar 3: Docker Lifecycle Logs** | `docker.log` | Container startup, lifecycle telemetry, and `[SYSTEM_BOOT]` markers. |
| **Pillar 4: Backend & DB Logs** | `app.log`, `sdk.log` | Request telemetry via `createSafeHandler`, rate-limiting events, and logger telemetry. |
