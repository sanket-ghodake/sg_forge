...
case "$CMD" in
    ...
    verify|lint|deadcode|secrets|arch|typecheck|shellcheck|semgrep|a11y|spectral|contracts|complexity|check-pkg|licenses|vuln|trivy|sbom|lhci|fuzz|schemathesis|loadtest|k6|benchmark|pack|graft|tokens|tokscale|headroom|council|diagram:lint|diagram-lint|diagram:convert-mermaid|mermaid-extract|diagram:convert-drawio|drawio-extract|docs:coverage|doc-coverage|docs:dev|docs:build)
