...
def main() -> int:
    ...
    if not files:
        diagram_dir = Path("apps/src/docs/public/diagrams")
        if not diagram_dir.exists():
            diagram_dir = Path("apps/src/docs/diagrams")
        if diagram_dir.exists():
            files = list(diagram_dir.glob("**/*.html"))
        if not files:
            print("No diagram HTML files found to lint in apps/src/docs/public/diagrams.")
            return 0
    for path in files:
        try:
            errors = verify(path)
