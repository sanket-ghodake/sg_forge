 ...
 * proxy health checks without requiring external PyPI compilation.
 */
...
function compressText(raw: string): string {
  // If JSON, parse and compact
  try {
    const parsed = JSON.parse(raw);
    return JSON.stringify(parsed);
  } catch {
    // Normal text or code: remove duplicate blank lines, collapse trailing whitespace
    return raw
      .replace(/\r\n/g, '\n')
      .replace(/[ \t]+$/gm, '')
      .replace(/\n{3,}/g, '\n\n')
...
function printStatus() {
  ...
  console.log(`   ├─ Proxy Port:        \x1b[35m8787\x1b[0m`);
  console.log(`   └─ Architecture:      Standalone Repo Toolchain`);
}
...
async function main() {
  ...
  switch (command) {
    ...
    case 'savings':
      ...
      console.log(`   └─ Status:                 Ready for proxy & agent integration\n`);
      break;
