---
trigger: always_on
description: Consult Headroom for prompt context compression, payload reduction, and local proxy acceleration.
---
...
2. **Local Compression Proxy**:
   - When running multi-agent workflows or streaming tool calls, Headroom can route traffic through `http://127.0.0.1:8787` (`./run.sh headroom proxy`) to compress prompts and enforce verbosity steering locally.
