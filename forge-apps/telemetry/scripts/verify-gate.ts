...
if (!existsSync(dockerfilePath) || !existsSync(composePath)) {
  failGate('06', 'Container & Dockerfile Standards', 'Missing docker/Dockerfile or docker-compose.yml');
} else {
  ...
  if (!dockerfile.includes('HEALTHCHECK') && !dockerfile.includes('health')) {
    ...
    passGate('06', 'Container & Dockerfile Standards', 'Standalone Dockerfile and compose stack verified.');
  }
}
